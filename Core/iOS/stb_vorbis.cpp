...
/** MODIFIED FOR MONOCLE AUDIO ENGINE (particularly, AudioAssetReader) by JOSH WHELCHEL **/
...
static uint8 get8(vorb *z)
{
    ...
    {
        unsigned char c;
        int s = z->reader->Read(&c,1);
        if (s==0) { z->eof = TRUE; return 0; }
        return c;
    }
#endif
}
...
#ifdef MONOCLE_STB
    {
        long x = z->reader->Tell();
        z->reader->Seek(x+n,SEEK_SET);
    }
#endif
}
...
static int set_file_offset(stb_vorbis *f, unsigned int loc)
{
    ...
    if (loc > f->reader->GetSize()){
        f->eof = 1;
        f->reader->Seek(f->f_start,SEEK_END);
        return 0;
    }
    
    f->reader->Seek(loc,SEEK_SET);
    return 1;
#endif
}
...
#ifdef MONOCLE_STB
    return f->reader->Tell() - f->f_start;
#endif
...
stb_vorbis * stb_vorbis_open_reader(AudioAssetReader *reader, int *error, stb_vorbis_alloc *alloc_buffer)
{
    stb_vorbis *f, p;
    if (reader == NULL) return NULL;
    vorbis_init(&p, alloc_buffer);
    p.reader = reader;
    p.stream_len = reader->GetSize();
    p.f_start = reader->Tell();
    p.push_mode = FALSE;
    if (start_decoder(&p)) {
        f = vorbis_alloc(&p);
        if (f) {
