/*
 * This file was automatically generated by AutoBind.
 * Any changes made to this file will be lost when it
 * is next regenerated.
 */

#include "autobind/types.h"
#include "autobind/binding/lua.h"
#include "Object.h"
#include "Entity.h"
#include "InternalConstructorOnlyException.h"

namespace Engine
{
    /* Variable assignments */

    /* Method and constructor definitions */
    Entity::Entity(lua_State * L, bool byuser)
    {
        if (byuser)
		{
			throw new Engine::InternalConstructorOnlyException();
		}
		// TODO: Implement.
	}

    /* Automatic dispatchers for overloaded methods */
    /* Automatic property getter-setter definitions */
    /* Binding variables */
    const char* Entity::ClassName = "Engine.Entity";
    const char* Entity::Inherits = "Object";
    const Bindings<Entity>::FunctionType Entity::Functions[] =
    {
        {0}
    };
    const Bindings<Entity>::PropertyType Entity::Properties[] =
    {
        {0}
    };
    int (__cdecl *Entity::Dispatcher)(lua_State * L) = &(Bindings<Entity>::FunctionDispatch);
} 

